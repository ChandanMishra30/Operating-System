...
{ 
      ...
      time_slice=1; 
      printf("\nProcess ID\t\tArrival Time\tBurst Time\tCompletion Time\t Turnaround Time\t Waiting Time\t Priority\n");
      for(total = 0, i = 0; x != 0;) 
      { 
	...
	for(b=0;b<limit;b++)
		{
            ...
            { 
                  x--; 
                  printf("\nProcess[%d]\t\t   %d\t\t    %d\t\t    %d\t\t   %d\t\t\t    %d\t\t   %d", p[i],a_time[i], b_time[i],a_time[i]+total - a_time[i], total - a_time[i], total - a_time[i] - b_time[i],prio[i]);
